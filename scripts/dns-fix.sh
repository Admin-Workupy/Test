...
main() {
  ...
  for container in ingress frontend; do
    ...
    case $container in
      ingress)
        sed -i "s/^DNS_HOST=.*/DNS_HOST=$domain/" "$ROOT_PATH/env/.env.nginx"
      ;;

      frontend)
        sed -i "s/^DNS_HOST=.*/DNS_HOST=$domain/" "$ROOT_PATH/env/.env.$container"
        sed -i "s/^VITE_DNS_HOST=.*/VITE_DNS_HOST=$domain/" "$ROOT_PATH/env/.env.$container"
      ;;
    esac
  done
